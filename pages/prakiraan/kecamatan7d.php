            ...
            <div class="container">
                <div class="row justify-content-between">
                    <div class="col-md form-floating mb-3">
                        <select name="selectKab" id="selectKab" aria-label="Floating label select example"
                            class="form-select col-md" onchange="kabupatenSelected()">
                    ...
                    <div class="col-md form-floating">
                        <select name="selectKec" id="selectKec" aria-label="Floating label select example"
                            class="form-select col-md" onchange="kecamatanSelected()"></select>
                        <label for="selectKec" style="margin-left:4px; font-size: 14px;">Choose Kecamatan</label>
                    </div>
                    <div class="col-md form-floating">
                        <select name="selectDate" id="selectDate" class="form-select col-md"
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $.each(response, function(key, value) {
                $("#selectKab").append('<option value=' + value.idkab + '>' + value.namakab +
                    '</option>');
            });
        }
    });
...
function kecamatanSelected() {
    ...
    $.ajax({
        ...
        success: function(response) {
            $("#selectKec").attr('disabled', false);
            $("#selectDate").empty()
            let data = [response.slice(0, 8), response.slice(8, 16), response.slice(16, 24), response.slice(
                24, 28), response.slice(28, 32), response.slice(32, 36), response.slice(36, 40)]
            var now = new Date;
            var ampm = 'pm';
            var allkota = [];
                                     ...
                                     <div class="bg-cuaca ${weathericon[element.weather].toLowerCase().replace(" ", "-")}-${ampm == "pm" ? "malam" : "siang"} " style="padding:10px; ">
                                        <div class="" >
                                            <div class="" style="display: flex; flex-direction:column;align-items: center;">
                                                <img style="width:60px; " src="/bmkgjuanda/assets/img/icon-cuaca/${weathericon[element.weather].toLowerCase()}-${ampm}.png" alt="${weathericon[element.weather].toLowerCase()}">
